...
function buildCurlExample({ method, path, body }) {
  const lines = [
    `curl -X ${method} "https://echo.smartspace.co.kr/api/sattie${path}"`,
...
const exampleUplinkResponse = {
  ...
  ground_station_id: null,
  ground_station_name: null,
  ground_station_type: null,
  requestor_id: null,
  requestor_name: null,
  mission_name: "prod terminal uplink test",
  aoi_name: "Seoul",
      ...
      "/uplink": {
        post: {
          ...
          description: buildOperationDescription({
            ...
            requestNotes:
              "satellite_id, mission_name, and aoi_name are mandatory. For direct production testing, start with the minimal payload shown below. ground_station_id and requestor_id should only be sent when they exactly match live operational records.",
            responseNotes:
              "The create response returns the initial QUEUED state immediately. Subsequent state transitions are observed through the commands endpoints.",
            method: "POST",
            path: "/uplink",
